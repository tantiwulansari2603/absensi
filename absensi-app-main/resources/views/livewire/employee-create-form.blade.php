    ...
    <form wire:submit.prevent="saveEmployees" method="post" novalidate>
        ...
        @foreach ($employees as $i => $employee)
            <div class="mb-3">
                <div class="w-100">
                    <div class="mb-3">
                        <x-form-label id="name{{ $i }}" label='Nama Peserta Magang {{ $i + 1 }}' />
                        <x-form-input id="name{{ $i }}" name="name{{ $i }}"
                            wire:model.defer="employees.{{ $i }}.name" />
                        <x-form-error key="employees.{{ $i }}.name" />
                    </div>
                    <div class="mb-3">
                        <x-form-label id="email{{ $i }}" label='Email Peserta Magang {{ $i + 1 }}' />
                    ...
                    <div class="mb-3">
                        ...
                        <x-form-input id="phone{{ $i }}" name="phone{{ $i }}"
                            wire:model.defer="employees.{{ $i }}.phone" placeholder="Format: 08**" />
                        <x-form-error key="employees.{{ $i }}.phone" />
                    </div>
                    <div class="mb-3">
                        <x-form-label id="password{{ $i }}"
                            label='Password Peserta Magang {{ $i + 1 }} (default: "123" jika tidak diisi)'
                            required="false" />
                ...
                @if ($i > 0)
                    <button class="btn btn-sm btn-danger mt-2" wire:click="removeEmployeeInput({{ $i }})"
                        wire:target="removeEmployeeInput({{ $i }})" type="button"
