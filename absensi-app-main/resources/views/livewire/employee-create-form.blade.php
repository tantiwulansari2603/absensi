        ...
        <div class="mb-3">
            <div class="w-100">
                ...
                <div class="mb-3">
                    ...
                    <x-form-input id="email{{ $i }}" name="email{{ $i }}" type="email" wire:model.defer="employees.{{ $i }}.email" placeholder="Email aktif" />
                    <x-form-error key="employees.{{ $i }}.email" />
                </div>
                <div class="mb-3">
                    <x-form-label id="phone{{ $i }}" label='No. Telp {{ $i + 1 }}' />
                    <x-form-input id="phone{{ $i }}" name="phone{{ $i }}" wire:model.defer="employees.{{ $i }}.phone" placeholder="Format: 08**" />
                    <x-form-error key="employees.{{ $i }}.phone" />
                </div>
                <div class="mb-3">
                    <x-form-label id="password{{ $i }}" label='Password User {{ $i + 1 }} (default: "123" jika tidak diisi)' required="false" />
                    <x-form-input id="password{{ $i }}" name="password{{ $i }}" wire:model.defer="employees.{{ $i }}.password" required="false" />
                    <x-form-error key=" employees.{{ $i }}.password" />
                </div>
                <div class="mb-3">
                    <x-form-label id="position_id{{ $i }}" label='Jabatan / Posisi {{ $i + 1 }}' />
                    <select class="form-select" aria-label="Default select example" name="position_id" wire:model.defer="employees.{{ $i }}.position_id">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="role_id" wire:model.defer="employees.{{ $i }}.role_id">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="schools_id" wire:model.defer="employees.{{ $i }}.schools_id">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="locations_id" wire:model.defer="employees.{{ $i }}.locations_id">
            ...
            @if ($i > 0)
            <button class="btn btn-sm btn-danger mt-2" wire:click="removeEmployeeInput({{ $i }})" wire:target="removeEmployeeInput({{ $i }})" type="button" wire:loading.attr="disabled">Hapus</button>
            @endif
        </div>
        <hr>
        @endforeach
